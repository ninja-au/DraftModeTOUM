...
namespace DraftModeTOUM.DraftTypes
{
    ...
    {
        public static BanDraftScreenController Instance { get; private set; }
        ...
        public static void Show(List<ushort> roleIds, bool allowPick)
        {
            ...
            var go = new GameObject("BanDraftScreenController");
        ...
        public static void Hide()
        {
            ...
            if (HudManager.Instance != null)
            {
                ...
                for (int i = hud.childCount - 1; i >= 0; i--)
                {
                    ...
                    if (child != null && child.name.StartsWith("BanDraftCard_"))
        ...
        private void BuildScreen()
        {
            ...
            {
                DraftModePlugin.Logger.LogWarning($"[BanDraftScreen] Bundle load failed: {ex.Message}");
            }

            if (prefab == null)
            {
                DraftModePlugin.Logger.LogError("[BanDraftScreen] SelectRoleGame prefab not found.");
                Destroy(gameObject); Instance = null; return;
            }

            _screenRoot = Instantiate(prefab);
            _screenRoot.name = "BanDraftRoleSelectScreen";
            ...
            if (_roleIds != null) idList.AddRange(_roleIds);
            var cards = DraftUiManager.BuildCards(idList, includeRandom: false);
            ...
            for (int i = 0; i < totalCards; i++)
            {
                ...
                ushort capturedRoleId = capturedIdx >= 0 && capturedIdx < idList.Count ? idList[capturedIdx] : (ushort)RoleTypes.Crewmate;
        ...
        private static IEnumerator CoAnimateCards(Transform rolesHolder, float cardScale, bool useGrid, int totalCards)
        {
            ...
            for (int i = 0; i < rolesHolder.childCount; i++)
            {
                ...
                catch (Exception bex) { DraftModePlugin.Logger.LogWarning($"[BanDraftScreen] BetterBloop failed: {bex.Message}"); }
        ...
        private void OnCardClicked(ushort roleId)
        {
